...
class AgentTools(llm.ToolContext):
    ...
    async def save_booking_intent(
        ...
        try:
            ...
            # Prevent duplicate bookings in the same call
            if self.booking_intent and self.booking_intent.get("confirmed"):
                return f"The booking for {effective_name} is already confirmed. No duplicate needed."

            self.caller_name = effective_name
            self._note_phone_confirmation(effective_phone)

            # Create booking IMMEDIATELY instead of deferring to shutdown hook
            logger.info("[TOOL] Creating booking immediately for %s at %s", effective_name, start_time)
            ...
            if result.get("success"):
                ...
                logger.info("[TOOL] Booking confirmed immediately: id=%s", booking_id)
                return f"Booking confirmed for {effective_name} at {start_time}. Booking ID: {booking_id}."
            else:
                error_msg = result.get("message", "Unknown booking error")
                logger.warning("[TOOL] Booking creation failed: %s", error_msg)
                return f"I could not complete the booking: {error_msg}. Please try a different time."
        except Exception as exc:
    ...
    async def unified_shutdown_hook(shutdown_ctx: JobContext) -> None:
        ...
        if agent_tools.booking_intent:
            ...
            if intent.get("confirmed"):
                # Booking was already created immediately during the call
                booking_was_confirmed = True
                booking_status_msg = f"Booking Confirmed: {intent.get('booking_id', 'N/A')}"
                logger.info("[SHUTDOWN] Booking was already confirmed during call: %s", intent.get("booking_id"))
            else:
                # Fallback: try creating booking now (legacy path)
                result = await async_create_booking(
        ...
        # Notify the dashboard API so SSE clients get instant updates
        try:
            import aiohttp
            api_port = parse_int(os.environ.get("PORT"), 8000)
            async with aiohttp.ClientSession() as http_session:
                await http_session.post(
                    f"http://localhost:{api_port}/api/internal/notify",
                    json={"event": "call_log_saved", "data": {"room": ctx.room.name, "booked": booking_was_confirmed}},
